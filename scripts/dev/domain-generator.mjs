...
async function updateDomainsIndex(domainName) {
  ...
  content = content.replace(
    ...
    (match, start, newline, existing, end) => {
      const updatedExisting = existing.trim()
        ? `${existing.trim()}\n  ${routeRegistration}`
        : routeRegistration;
...
const main = async () => {
  console.log(
    "Enter the domain name (use lowercase letters and hyphens for multiple words):"
  );
  ...
  if (!validateDomainName(trimmedDomainName)) {
    console.error(
      "Invalid domain name. Please use lowercase letters and hyphens for multiple words."
    );
    return;
  }
